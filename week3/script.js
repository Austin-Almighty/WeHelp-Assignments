...
function fetchData(url) {
  fetch(url)
    ...
    .then((data) => {
      ...
      const smallFirstDivs = document.querySelectorAll(".small.first"); // First 3 positions
      const titleDivs = document.querySelectorAll(".title"); // Next 10 positions

      // Populate the first 3 positions under `.small.first`
      for (let i = 0; i < smallFirstDivs.length && i < 3; i++) {
        const newDiv = document.createElement("div");
        newDiv.textContent = stitles[i];
        newDiv.className = "promo"; // Add class 'promo'
